...
use NomencladorBundle\Entity\ConsejoTecnico;

/**
 * @Route("nomenclador/consejo_tecnico/")
 */
class ConsejoTecnicoController extends Controller
{
    /**
     * List All values from ConsejoTecnico
     *
     * @Route("list")
     */
    public function listAction()
    {
        ...
        foreach($this->getDoctrine()->getManager()->getRepository('NomencladorBundle:ConsejoTecnico')->findAll() as $value ) {
            $data[] = array(
                'id' => $value->getId(), 
    ...
    public function addEditAction(Request $rq)
    {
        ...
        /* Add or Edit ConsejoTecnico */
        if ($rq->get('id')) {
            $entity = $em->getRepository('NomencladorBundle:ConsejoTecnico')->find($rq->get('id'));
        } else {
            $entity = new ConsejoTecnico();
        }
        /* Sets */
    ...
    {
        $em = $this->getDoctrine()->getManager();
        /* Delete ConsejoTecnico */
        foreach (json_decode($rq->get('ids')) as $id) {
            $entity = $em->getRepository('NomencladorBundle:ConsejoTecnico')->find($id);
            $em->remove($entity);
        }
        return new Response($em->flush());
    }
