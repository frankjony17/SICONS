...
use NomencladorBundle\Entity\Objetos;

/**
 * @Route("nomenclador/objetos/")
 */
class ObjetosController extends Controller
{
    /**
     * List All values from Objetos
     *
     * @Route("list")
     */
    public function listAction()
    {
        ...
        foreach($this->getDoctrine()->getManager()->getRepository('NomencladorBundle:Objetos')->findAll() as $value ) {
            $data[] = array(
                'id' => $value->getId(), 
    ...
    public function addEditAction(Request $rq)
    {
        ...
        /* Add or Edit Objetos */
        if ($rq->get('id')) {
            $entity = $em->getRepository('NomencladorBundle:Objetos')->find($rq->get('id'));
        } else {
            $entity = new Objetos();
        }
        /* Sets */
    ...
    public function removeAction(Request $rq)
    {
        ...
        foreach (json_decode($rq->get('ids')) as $id) {
            $entity = $em->getRepository('NomencladorBundle:Objetos')->find($id);
            $em->remove($entity);
        }
        return new Response($em->flush());
    }
