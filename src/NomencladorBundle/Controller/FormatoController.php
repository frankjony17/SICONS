...
use NomencladorBundle\Entity\Formato;

/**
 * @Route("nomenclador/formato/")
 */
class FormatoController extends Controller
{
    /**
     * List All values from Formato
     *
     * @Route("list")
     */
    public function listAction()
    {
        ...
        foreach($this->getDoctrine()->getManager()->getRepository('NomencladorBundle:Formato')->findAll() as $value ) {
            $data[] = array(
                'id' => $value->getId(), 
    ...
    public function addEditAction(Request $rq)
    {
        ...
        /* Add or Edit Formato */
        if ($rq->get('id')) {
            $entity = $em->getRepository('NomencladorBundle:Formato')->find($rq->get('id'));
        } else {
            $entity = new Formato();
        }
        /* Sets */
    ...
    public function removeAction(Request $rq)
    {
        ...
        foreach (json_decode($rq->get('ids')) as $id) {
            $entity = $em->getRepository('NomencladorBundle:Formato')->find($id);
            $em->remove($entity);
        }
        return new Response($em->flush());
    }
